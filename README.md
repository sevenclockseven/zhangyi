...
- 📦 **进销存管理** — 商品档案、采购入库、销售出库、收付款、库存报表、自动生成凭证
...
| 版本 | 日期 | 内容 |
|------|------|------|
| v0.10.6 | 2026-07-03 | 权限系统修复：登录返回book_permissions；listBooks按book_users过滤账套可见性；修复c.GetInt类型不匹配导致普通用户看不到账套；前端book store新增isAdmin/canWrite权限控制；所有页面写操作按钮加v-if权限守卫；系统设置管理tab仅admin可见；对抗性审查修复：listBooks显式类型断言、DB错误检查、App.vue onMounted try-catch、日志清理上限 |
...
| v0.10.1 | 2026-07-02 | 报表数据修复(sumUp累加/现金流量表列名修正) |
| v0.10.0 | 2026-07-02 | 进销存管理模块(商品/采购入库/销售出库/收付款/库存报表/自动生成凭证) |
| v0.9.3 | 2026-07-01 | 图表分析（收支趋势/费用构成/利润趋势ECharts）+ 图表导出PNG/打印；现金流量表改进（未标记凭证检测+勾稽校验+期初期末余额展示） |
| v0.9.2 | 2026-06-30 | 对抗性代码审查修复：BookWritable中间件生效（只读用户无法写入）；切换账套请求竞态取消；Books.vue内存泄漏修复；资产卡片book_id校验；多处错误吞没修复（事务回滚+前端错误反馈） |
