#!/usr/bin/env python3
"""Generate the updated Assets.vue with asset transaction tab, import/export, and status change"""
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="资产卡片" name="cards">
        ...
        <el-table :data="cards" border size="small" style="width: 100%">
          ...
          <el-table-column label="="100" align="right">
      ...
      <el-tab-pane label="折旧计提" name="depreciation">
        <div style="display: flex; gap: 12px; align-items: center; margin-bottom: 16px">
          ...
          <el-button type="success" @click="runDep">执行计提</el-button>
        </div>
        ...
        <div v-if="depTotal > 0" style="margin-top: 12px; font-weight: bold">
          本月应计提合计：{{ fmt(depTotal) }} 元（将自动生成会计凭证）
        </div>
      </el-tab-pane>
    ...
    <el-dialog v-model="showCardDialog" :title="cardForm.id ? '编辑资产' : '新增资产'" width="600px">
      <el-form :model="cardForm" label-width="100px">
        ...
        <el-form-item label="取得日期">
          <el-date-picker v-model="_date" value-format="YYYY-MM-DD" format="YYYY-MM-DD" style="width: 100%" />
        </el-form-item>
      ...
      <el-form :model="changeForm" label-width="80px">
        ...
        <el-form-item label="新状态" required>
          <el-select v-model="changeForm.status" placeholder="选择新状态" style="width: 100%">
            <el-option label="在用" value="in_use">
              <el-tag type="success" size="small">在用</el-tag>
            </el-option>
            <el-option label="闲置" value="idle">
              <el-tag type="info" size="small">闲置</el-tag>
            </el-option>
            <el-option label="维修" value="maintenance">
              <el-tag type="warning" size="small">维修</el-tag>
            </el-option>
            <el-option label="报废" value="scrapped">
              <el-tag type="danger" size="small">报废</el-tag>
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="存放地点">
        ...
        <el-form-item label="备注">
          <el-input v-model="changeForm.note" type="textarea" :rows="2" placeholder="变更原因或说明" />
        </el-form-item>
      </el-form>
      <template #footer>
...
async function loadCards() {
  ...
  try {
    ...
    cards.value =  } finally { loading.value = false }
}
...
async function runDep() {
  await ElMessageBox.confirm(`确定对 ${depPeriod.value} 执行折旧计提？系统将自动生成会计凭证。`, '确认', { type: 'warning' })
  const { data } = await assetApi.runDepreciation(bookId, depPeriod.value)
  ElMessage.success(`计提完成，共 ${data.count} 项资产，合计 ${fmt(data.total)} 元，已生成会计凭证`)
  depResults.value = []
  depTotal.value = 0
  loadCards()
  loadSummary()
...
async function saveStatus() {
  ...
  if (changeForm.status === changeForm.current_status) {
    ElMessage.warning('新状态与当前状态相同')
    return
  }
  await assetApi.changeStatus(bookId, changeForm.card_id, {
    status: changeForm.status,
    location: changeForm.location,
    department: changeForm.department,
    employee_name: changeForm.employee_name,
...
async function handleExport() {
  ...
  if (items.length === 0) {
    ElMessage.warning('暂无资产可导出')
    return
  }
  const headers = ['编号', '名称', '规格', '分类', '原值', '累计折旧', '净值', '月折旧',
    '状态', '部门', '责任人', '存放地点', '取得日期', '折旧起始月', '使用年限(月)', '净残值率', '来源', '备注']
  const fields = ['code', 'name', 'spec_model', 'category_name', 'original_value',
    'accumulated_depreciation', 'net_value', 'monthly_depreciation', 'status',
    'department', 'employee_name', 'location', 'acquisition_date',
    'depreciation_start_month', 'useful_life_months', 'residual_value_rate', 'source', 'remark']
...
with open('/opt/zhangyi/web/src/views/Assets.vue', 'w') as f:
    f.write(content)

print("OK: Assets.vue written successfully")
